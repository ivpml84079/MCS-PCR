...
bool WHTTC::regis(
	...
	if (verbose > 0) {
		std::cout << std::endl << "--- Processing Source Point Cloud ---" << std::endl;
	...
	if (verbose > 0) {
		...
		std::cout << "Source point cloud total processing time: " << src_proc_duration.count() << " ms" << std::endl << std::endl;
	}
	...
	if (verbose > 0) {
		std::cout << "--------------- Finish --------------" << std::endl;
		std::cout << "Target point cloud total processing time: " << tgt_proc_duration.count() << " ms" << std::endl << std::endl;
	}
	...
	if (verbose > 0) {
		std::cout << "\n--------- Building up Line-Intersection Graphs ---------" << std::endl;
	}
	...
	if (verbose > 1) {
		std::cout << "Source line-intersection graph node number: " << graph_src.nodes.size() << std::endl;
		std::cout << "Target line-intersection graph node number: " << graph_tgt.nodes.size() << std::endl;
	}
	...
	if (verbose > 0) {
		std::cout << "------------------------ Finish ------------------------" << std::endl;
		std::cout << "Line-intersection graphs building time: " << graph_build_duration.count() << " ms" << std::endl << std::endl;
	}

	if (verbose > 0) {
		std::cout << std::endl << "--- Identify Maximal Congruent Subgraph Pairs ---" << std::endl;
	}
	...
	if (verbose > 1) {
		std::cout << "Subgraph pair number: " << topk_tcsubgraphs.size() << std::endl;
		std::cout << "Matched node-pair number of first subgraph pair: " << topk_tcsubgraphs.front().node_pairs.size() << std::endl;
	}

	if (verbose > 0) {
		std::cout << "--------------------- Finish --------------------" << std::endl;
		std::cout << "Subgraph pairs identification time: " << subgraph_build_duration.count() << " ms" << std::endl << std::endl;
	}


	if (verbose > 0) {
		std::cout << std::endl << "------------ 2D Registration ------------" << std::endl;
	}
	...
	if (verbose > 1) {
		std::cout << "2D rotation:" << std::endl;
		std::cout << lb2dregister.getRotation2D() << std::endl;
		std::cout << "2D translation:" << std::endl;
		std::cout << lb2dregister.getTranslation2D() << std::endl;
	}

	if (verbose > 0) {
		std::cout << "----------------- Finish ----------------" << std::endl;
		std::cout << "2D registration time: " << loop_riges_duration.count() << " ms" << std::endl << std::endl;
	}
	
	if (successful == false) return false;

	if (verbose > 0) {
		std::cout << std::endl << "----------- Z-axis Estimation -----------" << std::endl;
	}
	auto z_est_bgn = std::chrono::high_resolution_clock::now();
	...
	auto z_est_end = std::chrono::high_resolution_clock::now();
	auto z_est_duration = std::chrono::duration_cast<std::chrono::milliseconds>(z_est_end - z_est_bgn);
	if (verbose > 0) {
		std::cout << "----------------- Finish ----------------" << std::endl;
		std::cout << "Z-axis estimation time: " << z_est_duration.count() << " ms" << std::endl << std::endl;
	}

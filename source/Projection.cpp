...
bool IFVRProcessor::process(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, int verbose) {
    ...
    std::vector<std::pair<Eigen::Vector3i, VoxelData>> all_voxels(voxels_.begin(), voxels_.end());
    std::vector<std::pair<Eigen::Vector3i, VoxelData>> valid_voxels;
    valid_voxels.reserve(all_voxels.size());

    for (int i = 0; i < static_cast<int>(all_voxels.size()); ++i) {
        auto& [voxel_idx, voxel_data] = all_voxels[i];
...
bool IFVRProcessor::voxelizePointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, int verbose) {
    for (int i = 0; i < cloud->points.size(); ++i) {
        ...
        voxels_[voxel_idx].points->push_back(point);
    }
